"""SQL queries for 5_Previsions (Forecasting and Fraud Loss)."""
...
def load_daily_series(history_days: int, series_choice: str, bank_label: str) -> pd.DataFrame:
    ...
    try:
        ...
        print("load_daily_series error:", exc)
        return pd.DataFrame()
...
def load_daily_fraud_series(history_days: int, series_choice: str, bank_label: str) -> pd.DataFrame:
    ...
    WHERE t.is_suspicious = true
      AND t.tx_step >= GREATEST(1, mx.s - {int(history_days)} + 1)
      {where_bank}
    GROUP BY 1
    ORDER BY 1;
    """
    try:
        ...
        print("load_daily_fraud_series error:", exc)
        return pd.DataFrame()
